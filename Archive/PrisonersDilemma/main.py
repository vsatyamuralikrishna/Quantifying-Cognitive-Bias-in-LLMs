...
def run_with_only_one_prisoner_knowing_partners_persona_and_partner_knows_his_persona_with_history(total_runs=10, persona: str = None, persona_dict: dict = None):
    messages=[
        {"role": "system", "content": basic_prompt},
        {"role": "user", "content": "Snitch on your partner"}
    ]
    ...
    for i in range(total_runs):
        if count == 1:
            ...
            agent2_messages = agent2_messages + [{"role": "system", "content": history_prompt}]
            agent1_messages = agent1_messages + [{"role": "system", "content": f"You know your partners persona as : {persona}"}]
            agent2_messages = agent2_messages + [{"role": "system", "content": f"Your persona is : {persona}"}]
            count = 0
        ...
        prisoner1_log = {
            ...
            "prisoner": 1,
            "response": resp1,
            "metadata" : "This prisoner knows the persona of the partner, but he is not given any persona but the partners knows his own persona"
        }
        prisoner2_log = {
            "conversation_id" : conversation_id,
            "prisoner": 2,
...
def run_with_both_prisoners_having_persona_only_one_knowing_about_other(total_runs=10, persona1: str = None, persona_dict1: dict = None, persona2: str = None, persona_dict2: dict = None):
    messages=[
        {"role": "system", "content": basic_prompt},
        {"role": "user", "content": "Snitch on your partner"}
    ]
...
#     details = [f"{key}: {value}" for key, value in persona.items()]
#     persona_string = ", ".join(details)
#     run_with_only_one_prisoner_knowing_partners_persona_and_partner_knows_his_persona_with_history(persona=persona_string, persona_dict=persona)
